<script>
  import logo from './assets/svelte.png'
  import Counter from './lib/Counter.svelte'
  import { onMount } from 'svelte';
  import axios from 'axios';

  let translatedText = ''; 

  onMount(async () => {
    let page = 1;
    let limit = 10;

    // async function fetchData() {
    //   const { data } = await axios.get('https://jsonplaceholder.typicode.com/posts?_page=${page}&_limit=${limit}`');
    //   console.log(data);
    // }

    let encodedText = encodeURIComponent('안녕하세요 너는 바보입니다');

    let config = {
      method: 'GET',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Authorization': 'KakaoAK fc8d321b4940c32f60419c42ecf59174',
      },
      dataType: 'json',
      url: ' https://dapi.kakao.com/v2/translation/translate?src_lang=kr&target_lang=en&query=' + encodedText,
    };

    async function fetchData() {
      //통신 가능 라이브러리.. const data 안에 리턴값 
      const { data } = await axios(config);
      console.log(data.translated_text[0]);
      translatedText = data.translated_text[0];
    }

    fetchData();
  });

</script>


<main>
  <img src={logo} alt="Svelte Logo" />
  <h1>{ translatedText } </h1>

  <Counter />

  <p>
    Visit <a href="https://svelte.dev">svelte.dev</a> to learn how to build Svelte
    apps.
  </p>

  <p>
    Check out <a href="https://github.com/sveltejs/kit#readme">SvelteKit</a> for
    the officially supported framework, also powered by Vite!
  </p>
</main>

<style lang="scss">
  @import "../public/global.scss";

  :root {
    font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen,
      Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif;
  }

  main {
    text-align: center;
    padding: 1em;
    margin: 0 auto;
  }

  img {
    height: 16rem;
    width: 16rem;
  }

  h1 {
    color: #ff3e00;
    text-transform: uppercase;
    font-size: 4rem;
    font-weight: 100;
    line-height: 1.1;
    margin: 2rem auto;
    max-width: 14rem;
  }

  p {
    max-width: 14rem;
    margin: 1rem auto;
    line-height: 1.35;
  }

  @media (min-width: 480px) {
    h1 {
      max-width: none;
    }

    p {
      max-width: none;
    }
  }
</style>
